...
**Build order (per `docs/tier-gating-spec.md` §Build order, unchanged and still correct):** (1) email-gated layer — ✅ SHIPPED 2026-07-03 (exports gated behind one-time email unlock: report/legal dialogs + evidence file; `EmailUnlock.tsx`, waitlist source `report_unlock`, event `report_unlock_submitted`, localStorage flag `a11y-beast:report-unlock`; verified live incl. axe-0 with `.verify-emailgate.mjs`) → (2) auth/accounts → (3) Stripe checkout + subscriptions → (4) hosted evidence ledger (localStorage v0 → server store) → (5) full-site crawl gating (flag already exists: `ENABLE_SITE_CRAWL`) → (6) styled PDF export of the evidence record (`docs/templates/evidence-record-template.html`). Agency white-label + multi-client ships after Pro.
